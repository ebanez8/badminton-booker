...
export function desiredDateTime(request: BookingRequest): DateTime {
  ...
  if (!value.isValid) throw new BookingError('VALIDATION_ERROR', 'Invalid Toronto booking date/time')
  return value
}

export function calculateReleaseDateTime(request: BookingRequest): DateTime {
  if (request.releaseRule.mode === 'exact-date-time') {
    const exact = DateTime.fromISO(request.releaseRule.dateTime ?? '', { setZone: true }).setZone(TORONTO_ZONE)
